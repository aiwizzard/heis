...
Plans start at $49/mo. [Get started with White Label →](https://muapi.ai/white-label?utm_source=github&utm_medium=readme&utm_campaign=heis)
...
**Hosted version:** [https://muapi.ai/heis?utm_source=github&utm_medium=readme&utm_campaign=heis](https://muapi.ai/heis?utm_source=github&utm_medium=readme&utm_campaign=heis)
...
- **AppImage** — portable, run directly after making executable:
  ```bash
  chmod +x "release/heis-*.AppImage"
  ./release/heis-*.AppImage
  ```
- **.deb** — install on Debian/Ubuntu:
  ```bash
  sudo apt install ./release/heis_*_amd64.deb
  ```
...
- macOS: `~/Library/Application Support/heis/local-ai`
- Windows: `%APPDATA%\heis\local-ai`
- Linux: `~/.config/heis/local-ai`

To keep multi-GB model weights on another drive, set `HEIS_LOCAL_AI_DIR`
...
APP_DATA="${HEIS_LOCAL_AI_DIR:-$HOME/Library/Application Support/heis/local-ai}"
...
> **Most users want the desktop app, not this dev path.** If you just want to run heis on your machine, [download a prebuilt installer](#-download-desktop-app) instead — no Node.js required. The instructions below are for contributors building from source.
...
The `packages/studio` library is also consumed by the hosted version on [muapi.ai](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=heis) — model updates made in `packages/studio/src/models.js` apply to both the self-hosted app and the hosted version automatically.

## 🔌 API Integration

The app communicates with [Muapi.ai](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=heis) using a two-step pattern:
...
**heis** is a community-driven, open-source alternative that provides similar creative capabilities without the closed ecosystem:

| | Other providers | heis |
| :--- | :--- | :--- |
...
| **Hosted version** | Subscription | Free at [muapi.ai/heis](https://muapi.ai/heis?utm_source=github&utm_medium=readme&utm_campaign=heis) |
| **Self-hosting** | No | Yes |
...
Built with [Muapi.ai](https://muapi.ai?utm_source=github&utm_medium=readme&utm_campaign=heis) — the unified API for AI image and video generation models.
