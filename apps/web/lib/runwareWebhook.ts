    ...
    if (data.error || data.errors?.length) {
      ...
      if (asset.url) {
        ...
        if (!existingAsset.data) {
          ...
          const stored = await admin.from("media_assets").insert({ user_id: job.user_id, job_id: job.id, kind: asset.kind, object_key: copied.objectKey, source_url: asset.url, mime_type: copied.contentType, size_bytes: copied.sizeBytes });
