...
try {
  ...
  await page.getByText("Advanced crop", { exact: true }).click();
  ...
  const cropDialog = page.getByRole("dialog", { name: "Crop preview" });
  await cropDialog.waitFor();
  await page.getByRole("button", { name: "Crop layout", exact: true }).click();
  await page.screenshot({path:"test-results/editor/crop-popover.png"});
  await page.keyboard.press("Escape");
  assert.equal(await cropDialog.isVisible(), true);
  assert.equal(await page.getByRole("button", {name:"Square 1:1",exact:true}).isVisible(), false);
  await page.getByRole("button", { name: "Crop layout", exact: true }).click();
  await page.getByRole("button", { name: "Square 1:1", exact: true }).click();
  const cropBox = page.locator(".heis-crop-box");
  await page.waitForFunction(() => Math.abs(Number(document.querySelector(".heis-crop-box")?.getAttribute("data-crop-ratio")) - 1) < 0.001);
  assert.ok(Math.abs(Number(await cropBox.getAttribute("data-crop-ratio")) - 1) < 0.001);
  const cropHandle = await page.getByRole("button", { name: "Crop bottom right handle", exact: true }).boundingBox();
  const initialCropBox = await cropBox.boundingBox();
  await page.mouse.move(cropHandle.x + 6, cropHandle.y + 6);
  await page.mouse.down();
  await page.mouse.move(cropHandle.x - 30, cropHandle.y - 25, { steps: 5 });
  await page.mouse.up();
  assert.ok((await cropBox.boundingBox()).width < initialCropBox.width);
  assert.ok(Math.abs(Number(await cropBox.getAttribute("data-crop-ratio")) - 1) < 0.001);
  await page.screenshot({ path: "test-results/editor/preview-crop.png" });
  await page.keyboard.press("Escape");
  assert.equal(await cropDialog.count(), 0);
  assert.equal(await page.getByRole("slider", { name: "Crop left", exact: true }).inputValue(), "0");
  await page.getByRole("button", { name: "Crop", exact: true }).click();
  await page.getByRole("button", { name: "Crop layout", exact: true }).click();
  ...
  await page.waitForFunction(() => Number(document.querySelector('[id="transform-Crop left"]').value) > 0);
  await page.getByRole("button", { name: "↶", exact: true }).click();
  await page.waitForFunction(() => document.querySelector('[id="transform-Crop left"]').value === "0");
  await page.getByRole("button", { name: "↷", exact: true }).click();
  await page.waitForFunction(() => Number(document.querySelector('[id="transform-Crop left"]').value) > 0);
