...
try {
  ...
  await page.getByRole("button", { name: "Crop on preview", exact: true }).click();
  const cropDialog = page.getByRole("dialog", { name: "Crop preview" });
  await cropDialog.waitFor();
  await page.getByRole("combobox", { name: "Crop aspect ratio" }).selectOption("1:1");
  ...
  await page.getByRole("button", { name: "Crop on preview", exact: true }).click();
  await page.getByRole("combobox", { name: "Crop aspect ratio" }).selectOption("9:16");
