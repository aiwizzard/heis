...
const TOOLS = [
  {name:'heis_workflow_info',description:'Read the scoped workflow graph, revision, runs, project assets and available managed capability input/output types.',inputSchema:{type:'object',properties:{}},annotations:{readOnlyHint:true}},
  {name:'heis_workflow_save',description:'Save an undoable complete graph at the expected revision. Every node needs id, kind, name, prompt, duration (5 or 10), x and y. Managed nodes need modelId, ordered sources for media ports, optional promptSource for text, sourceIndices (zero-based), and optional options. Input media needs assetId. Outputs need source. All nodes must reach an output. Read workflow_info first.',inputSchema:{type:'object',required:['revision','name','nodes'],properties:{revision:{type:'integer'},name:{type:'string'},nodes:{type:'array',items:{type:'object'}}}},annotations:{readOnlyHint:false}},
  {name:'heis_workflow_run',description:'Run the scoped workflow after native spending approval. Never inserts timeline media automatically.',inputSchema:{type:'object',required:['revision'],properties:{revision:{type:'integer'}}},annotations:{readOnlyHint:false,openWorldHint:true}},
  {name:'heis_workflow_cancel',description:'Cancel the scoped workflow run and its remaining jobs.',inputSchema:{type:'object',required:['runId'],properties:{runId:{type:'string'}}},annotations:{readOnlyHint:false}},
  ...
  { name: "heis_edit", description: "Apply an undoable edit batch to the open video project. Read heis_project_info first and use its project ID and revision. Supported edits: clip.update {sequenceId,id,patch}, clip.add {sequenceId,clip}, clip.split {sequenceId,id,frame,newId}, clip.remove {sequenceId,ids,ripple}, track.add {sequenceId,track}, track.update {sequenceId,id,patch}, sequence.add {sequence}, sequence.select {id}, sequence.update {id,patch}, rename {name}. Each edit has a type field. Read clip and track shapes from project_info. Color correction uses clip.update patch.color with all fields: exposure (-3 to 3 stops), contrast/highlights/shadows/temperature/tint (-100 to 100, neutral 0), saturation (0 to 200, neutral 100). Color edits are nondestructive and apply to preview and export. Times are integer frames. Asset registration uses import, not edit commands.", inputSchema: { type: "object", required: ["projectId", "expectedRevision", "label", "edits"], properties: { projectId: {type:"string"}, expectedRevision:{type:"integer"}, label:{type:"string"}, edits:{type:"array",items:{type:"object"}} } }, annotations: { readOnlyHint:false } },
